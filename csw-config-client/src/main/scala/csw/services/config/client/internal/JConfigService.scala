...
class JConfigService(configAdminService: ConfigAdminService, actorRuntime: ActorRuntime) extends IConfigService {
  ...
  override def create(path: Path,
    ...
    configAdminService.create(path, configData, annex, comment).toJava.toCompletableFuture
  ...
  override def update(path: Path, configData: ConfigData, comment: String): CompletableFuture[ConfigId] =
    configAdminService.update(path, configData, comment).toJava.toCompletableFuture
  ...
  override def getById(path: Path, id: ConfigId): CompletableFuture[Optional[ConfigData]] =
    configAdminService.getById(path, id).map(_.asJava).toJava.toCompletableFuture

  override def getLatest(path: Path): CompletableFuture[Optional[ConfigData]] =
    configAdminService.getLatest(path).map(_.asJava).toJava.toCompletableFuture

  override def getByTime(path: Path, time: Instant): CompletableFuture[Optional[ConfigData]] =
    configAdminService.getByTime(path, time).map(_.asJava).toJava.toCompletableFuture

  override def exists(path: Path): CompletableFuture[jl.Boolean] =
    configAdminService.exists(path).toJava.toCompletableFuture.asInstanceOf[CompletableFuture[jl.Boolean]]

  override def exists(path: Path, id: Optional[ConfigId]): CompletableFuture[jl.Boolean] =
    configAdminService.exists(path, id.asScala).toJava.toCompletableFuture.asInstanceOf[CompletableFuture[jl.Boolean]]

  override def delete(path: Path, comment: String): CompletableFuture[Unit] =
    configAdminService.delete(path, comment).toJava.toCompletableFuture
  ...
  override def list(fileType: FileType, pattern: String): CompletableFuture[util.List[ConfigFileInfo]] =
    configAdminService.list(Some(fileType), Some(pattern)).map(_.asJava).toJava.toCompletableFuture

  override def list(fileType: FileType): CompletableFuture[util.List[ConfigFileInfo]] =
    configAdminService.list(Some(fileType)).map(_.asJava).toJava.toCompletableFuture

  override def list(pattern: String): CompletableFuture[util.List[ConfigFileInfo]] =
    configAdminService.list(pattern = Some(pattern)).map(_.asJava).toJava.toCompletableFuture

  override def list(): CompletableFuture[util.List[ConfigFileInfo]] =
    configAdminService.list().map(_.asJava).toJava.toCompletableFuture

  override def history(path: Path, maxResults: Int): CompletableFuture[util.List[ConfigFileRevision]] =
    configAdminService.history(path, maxResults).map(_.asJava).toJava.toCompletableFuture
  ...
  override def setActive(path: Path, id: ConfigId, comment: String): CompletableFuture[Unit] =
    configAdminService.setActive(path, id, comment).toJava.toCompletableFuture
  ...
  override def resetActive(path: Path, comment: String): CompletableFuture[Unit] =
    configAdminService.resetActive(path, comment).toJava.toCompletableFuture
  ...
  override def getActive(path: Path): CompletableFuture[Optional[ConfigData]] =
    configAdminService.getActive(path).map(_.asJava).toJava.toCompletableFuture

  override def asScala: ConfigAdminService = configAdminService
}
