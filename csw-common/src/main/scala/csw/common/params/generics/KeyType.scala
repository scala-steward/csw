...
import csw.common.params.models.Units.second
import csw.common.params.models.{Units, _}
import csw.common.params.pb.ItemsFactory
...
sealed class KeyType[S: Format: ClassTag: ItemsFactory] extends EnumEntry with Serializable {
  private[common] def paramFormat: Format[Parameter[S]]                 = Parameter[S]
  private[common] def typeMapper: TypeMapper[PbParameter, Parameter[S]] = Parameter.typeMapper
}
...
object KeyType extends Enum[KeyType[_]] with PlayJsonEnum[KeyType[_]] {
  ...
  /**
   * values return a Seq of all KeyTypes provided by `csw-common`
   */
