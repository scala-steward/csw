...
import csw.common.location.ConnectionType.{AkkaType, HttpType, TcpType}
...
object Connection {
  ...
  private[common] implicit val connectionReads: Reads[Connection]   = ConnectionInfo.connectionInfoFormat.map(Connection.from)
  private[common] implicit val connectionWrites: Writes[Connection] = Writes[Connection](c ⇒ Json.toJson(c.connectionInfo))
