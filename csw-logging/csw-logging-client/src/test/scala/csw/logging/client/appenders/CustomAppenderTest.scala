...
class CustomAppenderBuilderClass extends LogAppenderBuilder {
  ...
  override def apply(system: ActorSystem[_], standardHeaders: JsObject): LogAppender =
...
class CustomAppender(system: ActorSystem[_], stdHeaders: JsObject, callback: Any ⇒ Unit) extends LogAppender {
...
class CustomAppenderTest extends FunSuite with Matchers {
  ...
  test("should be able to add and configure a custom appender using an object extending from CustomAppenderBuilder") {
    ...
    val actorSystem   = ActorSystem(SpawnProtocol.behavior, "test", config.resolve())
