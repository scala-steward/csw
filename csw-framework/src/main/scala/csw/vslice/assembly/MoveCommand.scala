...
class MoveCommand(ac: AssemblyContext,
  ...
  override def onMessage(msg: TromboneCommandMsgs): Behavior[TromboneCommandMsgs] = {
    msg match {
...
object MoveCommand {
  def make(ac: AssemblyContext,
    ...
    Actor.mutable(ctx ⇒ new MoveCommand(ac, s, tromboneHCD, startState, stateActor, ctx))
}
