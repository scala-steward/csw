...
class JsonTest extends FunSpec {
  ...
  describe("Testing Items") {
    ...
    it("should encode and decode items list") {
      ...
      val sc1   = Setup(runId, obsId, Prefix(ck)).add(i1).add(i2)
  ...
  describe("Commands and Events JSON") {
    ...
    it("Should encode/decode a Setup") {
      val c1 = Setup(runId, obsId, Prefix(ck)).add(i1).add(i2).add(i3).add(i4).add(i5).add(i6).add(i7)
      assert(c1.size == 7)
    ...
    it("Should encode/decode an Observe") {
      val c1 = Observe(runId, obsId, Prefix(ck)).add(i1).add(i2).add(i3).add(i4).add(i5).add(i6).add(i7)
      assert(c1.size == 7)
      val c1out = JsonSupport.writeSequenceCommand(c1)
    ...
    it("Should encode/decode an Wait") {
      val c1 = Wait(runId, obsId, Prefix(ck)).add(i1).add(i2).add(i3).add(i4).add(i5).add(i6).add(i7)
      assert(c1.size == 7)
      val c1out = JsonSupport.writeSequenceCommand(c1)
  ...
  describe("Test Custom RaDecItem") {
    it("Should allow custom RaDecItem") {
      ...
      val i1  = k1.set(c1, c2)
      val sc1 = Setup(runId, obsId, Prefix(ck)).add(i1)
      ...
      val sc2 = Setup(runId, obsId, Prefix(ck)).add(k1.set(c1, c2))
      assert(sc2 == sc1)
    }
  }
  ...
  describe("Test Double Matrix items") {
    it("Should allow double matrix values") {
      ...
      val sc1 = Setup(runId, obsId, Prefix(ck)).add(k1.set(m1))
      ...
      val sc2 = Setup(runId, obsId, Prefix(ck)).add(k1.set(m1))
      assert(sc2 == sc1)
    }
  }
  ...
  describe("Test Choice items") {
    it("Should allow choice/enum values") {
      ...
      val sc2 = Setup(runId, obsId, Prefix(ck)).add(k1.set(c1))
      assert(sc2 == sc1)
    }
  }
