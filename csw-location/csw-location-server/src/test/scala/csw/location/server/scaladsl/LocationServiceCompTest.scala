...
import csw.location.server.commons.{ClusterAwareSettings, NetworksUtil}
...
class LocationServiceCompTest(mode: String)
  ...
  test("should able to register, resolve, list and unregister tcp location") {
    ...
    locationService.resolve(connection, 2.seconds).await.get shouldBe tcpRegistration.location(NetworksUtil().hostname)
    locationService.list.await shouldBe List(tcpRegistration.location(NetworksUtil().hostname))
  ...
  test("should able to register, resolve, list and unregister http location") {
    ...
    locationService.list.await shouldBe List(httpRegistration.location(NetworksUtil().hostname))
  ...
  test("should able to register, resolve, list and unregister akka location") {
    ...
    locationService.list.await shouldBe List(akkaRegistration.location(NetworksUtil().hostname))
  ...
  test("akka location death watch actor should unregister services whose actorRef is terminated") {
    ...
    locationService.list.await shouldBe List(
      AkkaRegistration(connection, prefix, actorRef).location(NetworksUtil().hostname)
    )
  ...
  test(
    ...
    probe.expectMessage(LocationUpdated(redis1Registration.location(NetworksUtil().hostname)))
  ...
  test("should filter connections based on hostname") {
    ...
    locationService.list(NetworksUtil().hostname).await.map(_.connection).toSet shouldBe Set(tcpConnection,
                                                                                             httpConnection,
                                                                                             akkaConnection)
